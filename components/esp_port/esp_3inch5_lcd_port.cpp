...
#define EXAMPLE_PIN_LCD_MISO GPIO_NUM_NC // GPIO_NUM_13
#define EXAMPLE_PIN_LCD_MOSI GPIO_NUM_11
#define EXAMPLE_PIN_LCD_SCLK GPIO_NUM_12
#define EXAMPLE_PIN_LCD_CS   GPIO_NUM_10
#define EXAMPLE_PIN_LCD_DC   GPIO_NUM_46
#define EXAMPLE_PIN_LCD_RST  GPIO_NUM_NC
#define EXAMPLE_PIN_LCD_BL   GPIO_NUM_45

#define EXAMPLE_PIN_TP_INT   GPIO_NUM_17
#define EXAMPLE_PIN_TP_RST   GPIO_NUM_18
...
extern volatile SemaphoreHandle_t lcd_trans_done_sem;
...
bool notify_lcd_trans_done_cb(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_io_event_data_t *edata, void *user_ctx)
{
    BaseType_t high_task_wakeup = pdFALSE;

    // 비동기 전송이 완료되었으므로 세마포어를 반환하여 대기 중인 Task를 깨움
    if (lcd_trans_done_sem != NULL) {
        xSemaphoreGiveFromISR(lcd_trans_done_sem, &high_task_wakeup);
    } else {
    }

    // 더 높은 우선순위의 Task가 깨어났다면 문맥 교환(Context Switch) 요청
...
void esp_3inch5_display_port_init(esp_lcd_panel_io_handle_t *io_handle, esp_lcd_panel_handle_t *panel_handle, size_t max_transfer_sz)
{
    ...
    io_config.on_color_trans_done = notify_lcd_trans_done_cb; //NULL;
